...
public class DTHBrowsePlanType extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (DetectConnection.checkInternetConnection(DTHBrowsePlanType.this)) {
    ...
    private void setupViewPager(ViewPager viewPager,String data) {
        ...
        try {
            ...
            for (int i=0; i<jsonArray.length(); i++) {
                ...
                adapter.addFrag(new DTHPlansFragment().newInstance(jsonObject.getString("plantype_id"),provider,circle), jsonObject.getString("plan_title"));
            }
        }
    ...
    private  void  mGetBrowsPlanData(final String provider, final String circle) {
        class getJSONData extends AsyncTask<String, String, String> {
            ...
            protected void onPreExecute() {
                super.onPreExecute();
                dialog=new ProgressDialog(DTHBrowsePlanType.this);
...
//                    URL url = new URL(BaseURL.BASEURL_B2C+"api/plans/v1/prepaid-plans?api_token="+SharePrefManager.getInstance(BrowsePlan.this).mGetApiToken()+"&provider_id="+provider+"&state_id="+circle);
                    URL url = new URL(BaseURL.BASEURL_B2C+"api/plans/v1/type?api_token="+SharePrefManager.getInstance(DTHBrowsePlanType.this).mGetApiToken()+"&provider_id="+provider);
                    Log.e("sending",BaseURL.BASEURL_B2C+"api/plans/v1/type?api_token="+SharePrefManager.getInstance(DTHBrowsePlanType.this).mGetApiToken()+"&provider_id="+provider);
...
//                plans_data=result;
                ...
                try {
                    ...
                    if (!status.equals(""))
                    {
                        ...
                        {
                            Toast.makeText(DTHBrowsePlanType.this, message, Toast.LENGTH_SHORT).show();
                        }
                        else
                        {
                            Toast.makeText(DTHBrowsePlanType.this, "Something went wrong", Toast.LENGTH_SHORT).show();
                        }
                    }
                    else if (!message.equals(""))
                    {
